...
export const TaskQSortDropdown = () => {
  const { filters, setFilter } = useTaskQ();
  const [open, setOpen] = useState(false);

  const currentSortBy = filters.sortBy || "TaskDueDate";
  const currentOrder = filters.sortOrder || "desc";

  const sortOptions = [
    { label: "Default", key: "None" },

    { label: "Due Date", key: "TaskDueDate" },
    { label: "Task ID", key: "TaskID" },
    { label: "Title", key: "TaskTitle" },
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8 text-slate-700 hover:text-slate-700 hover:bg-slate-200"
        >
